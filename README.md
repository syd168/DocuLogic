...
# 2. 一键部署（会先检查 converts/models 两个目录是否已准备）
./docker/deploy.sh
...
### v4.3.0 (最新)
- ✨ 前端 Workspace 拆分：解析、系统设置等面板独立为组件，逻辑下沉至与组件同名的 composables，样式收口至 `frontend/src/styles/Workspace.css`
- ✨ 系统设置 UI：注册与安全手风琴、路径与模型等布局与 scoped 穿透修正，保存区与分栏风格统一
- 🐛 多页 PDF 合并 Markdown 时不再插入 `---` 页间分隔，避免渲染为多余横线（`web/app/model_inference.py`）
- 🔧 前端 `jsconfig.json` 与 PDF worker 等配套小调整

### v2.4.2
- ✨ 输出文件扩展名标准化：新任务默认使用 `.md`（兼容历史 `.mmd` 下载与批量打包）
- ✨ 模型卸载稳定性增强：补充内部权重/处理器释放流程，改进 CUDA 显存回收
- ✨ 批量下载与临时文件清理修复：统一请求体契约并确保响应后删除临时 ZIP
- ✨ 并发任务下模型生命周期优化：以引用计数替代布尔标志，避免推理中误卸载
- ✨ 文档与部署说明对齐：根 README 与 docker/README 统一，补充 CORS 与端口说明

### v2.4.1
- ✨ 修复中止转换后 ZIP 下载链接缺失问题
- ✨ 优化脚本文件结构，创建 scripts/ 目录统一管理运维工具
- ✨ 修复 MariaDB/PostgreSQL 网络别名问题，支持无缝切换数据库类型
- ✨ 增强数据库切换时的数据迁移提示，防止配置丢失
- 🔧 重命名 migrate_sqlite_to_mysql.py 为 convert_sqlite_to_other.py，更清晰通用
- 🐛 修复类型转换函数缩进错误，添加 PostgreSQL 支持
