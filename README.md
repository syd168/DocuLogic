...
- **🎯 强大解析能力**：基于 Logics-Parsing-v2 端到端大模型，支持 STEM 内容（公式、化学结构）、复杂版面、Parsing-2.0（流程图、乐谱）
...
- 部署前请先准备本地 `converts/models/` 目录（至少包含一个文档解析器源码子目录；该目录不提交到 GitHub）
- `converts/models` 推荐来源：`Logics-Parsing`（[GitHub](https://github.com/alibaba/Logics-Parsing)）或其他文档解析引擎源码
...
**部署脚本自动完成：**
- ✅ 检查 Docker、GPU 环境
- ✅ 创建数据目录（~/doculogic/）；模型默认放在 **`~/doculogic/weights/<文档解析器名>/`**（容器内为 **`/app/weights/<文档解析器名>/`**，`MODEL_PATH` 指向含 `config.json` 的快照目录，详见 [docker/README.md](docker/README.md)）
...
# 5. 启动后在后台「模型配置」中下载权重（下载路径统一为 weights/文档解析器名称）
...
**AI 引擎**：Logics-Parsing-v2（端到端大模型解析）
...
│       ├── models.py        # 数据模型
│       └── main.py          # 入口文件
├── converts/models/         # 本地准备的文档解析器源码目录（不提交到 GitHub，部署前必备）
├── weights/logics-parsing-v2/         # 当前默认文档解析器模型目录（下载/加载统一路径）
└── README.md
```
...
| 变量名 | 说明 | 示例 |
|--------|------|------|
| `MODEL_PATH` | 模型权重路径（当前默认文档解析器） | `./weights/logics-parsing-v2` |
