...
- **ORM**：SQLAlchemy 2.0（支持 SQLite/MySQL/PostgreSQL/MariaDB）
...
### 1. **企业级安全架构**
- 🔐 **多层安全防护**：JWT + SSO + Token 黑名单 + 异地登录检测
- 🛡️ **7个安全响应头**：CSP、HSTS、X-Frame-Options 等全面防护
- 🔒 **数据脱敏**：日志自动掩码邮箱、手机号、用户名
- ⚠️ **友好错误提示**：服务端详细记录，用户仅见安全提示
- 🚫 **SSRF 防护**：SMS HTTP URL 协议校验与内网拦截

### 2. **智能化文档解析**
- 🧠 **端到端单模型**：无需复杂流水线，一次推理完成解析
- 📊 **STEM 内容识别**：公式、化学结构（SMILES）、手写文本
- 🎨 **Parsing-2.0**：流程图（Mermaid）、乐谱（ABC）、伪代码
- 📑 **复杂版面处理**：多栏报纸、杂志、学术论文
- 🖼️ **多种输出模式**：Base64 嵌入、独立文件、不输出图片

### 3. **灵活的部署方案**
- 🐳 **Docker 一键部署**：5分钟快速启动，自动配置所有组件
- 💾 **多数据库支持**：SQLite（开发）/ MySQL（生产）无缝切换
- 🔄 **自动备份**：每天凌晨 2 点定时备份，保留 7 天
- 🚀 **GPU 加速**：NVIDIA CUDA 支持，推理速度提升 5-10 倍
- 📦 **资源限制**：可配置内存和 CPU 限制，防止资源耗尽

### 4. **完善的用户管理**
- 👤 **双因子认证**：图形验证码 + 邮箱/手机验证码
...
### 5. **实时监控与管理**
- 📈 **WebSocket 实时进度**：解析进度实时推送
- 📋 **任务历史查询**：完整的任务记录和结果下载
- 🗑️ **批量清理**：支持按时间范围清理旧任务
- ⚙️ **动态系统配置**：无需重启即可调整参数
- 📧 **邮件/SMS 服务**：支持 SMTP 和自定义 HTTP 接口

### 6. **优秀的用户体验**
- 🎯 **拖拽上传**：支持批量上传，可多次添加
- 📝 **可视化预览**：PNG/ZIP 格式预览解析结果
- 🎨 **语义化图标**：所有配置项都有对应的 emoji 图标
- 💬 **友好提示**：被踢下线、配置验证等场景的清晰提示
- 📱 **响应式设计**：适配桌面和移动设备
...
#### 1. **单点登录友好提示**
- ✅ **异地登录检测**：自动检测账号在其他设备登录
- ✅ **友好提示对话框**：被踢下线时显示安全警告
- ✅ **智能错误识别**：区分 Token 过期和被踢下线
- ✅ **安全建议**：提示用户如非本人操作立即修改密码

#### 2. **文件上传精细控制**
- ✅ **多文件上传开关**：管理员可开启/关闭多文件上传功能
- ✅ **自定义上传大小**：支持 1-500MB 范围自定义
- ✅ **环境变量配置**：通过 `MAX_UPLOAD_SIZE_MB` 设置后端上限
- ✅ **Nginx 联动**：实时验证 Nginx 配置与后端限制匹配
- ✅ **动态提示**：根据配置显示不同的上传说明文字

#### 3. **UI/UX 优化**
- ✅ **折叠面板图标**：为所有配置项添加语义化 emoji 图标
- ✅ **视觉层次清晰**：🔐 基础、🛡️ 验证码、👤 注册/登录、⚙️ 解析限制等
- ✅ **统一设计风格**：所有标题左侧都有对应功能的图标
- ✅ **交互体验提升**：更直观的界面导航
