#!/usr/bin/env python3
"""
跨平台启动 / 停止 DocuLogic 后端（FastAPI）与前端（Vite）。
...
或直接:
  python3 project_ctl.py start
  python3 project_ctl.py stop
...
def start() -> None:
    ...
    print(f"模型路径: {os.environ.get('MODEL_PATH', str(ROOT / 'weights'))}（可通过 MODEL_PATH 环境变量或系统设置修改）")
    print("PID file:", PID_FILE)
    print("Stop with: python project_ctl.py stop")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="DocuLogic dev processes")
