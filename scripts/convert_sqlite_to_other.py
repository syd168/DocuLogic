...
功能：
- 从 SQLite 数据库导出数据
- 生成兼容其他数据库（MySQL/MariaDB/PostgreSQL）的 SQL 文件
- 支持在本地或 Docker 环境中导入

使用方法：
    # 1. 导出 SQLite 数据为 SQL 文件
    python convert_sqlite_to_other.py export
    
    # 2. 导入到目标数据库（需要设置环境变量）
    # MySQL/MariaDB:
    export DB_TYPE=mysql
    export DB_PASSWORD=your_password
    python convert_sqlite_to_other.py import
...
环境变量：
    DB_TYPE: 目标数据库类型 (mysql/mariadb/postgresql，默认：mysql)
    DB_HOST: 数据库主机（默认：localhost）
    DB_PORT: 数据库端口（MySQL默认：3306，PostgreSQL默认：5432）
    DB_USER: 数据库用户名（默认：root）
    DB_PASSWORD: 数据库密码（导入时必需）
    DB_DATABASE: 数据库名（默认：doculogic）
"""
...
def convert_sqlite_type_to_target(sqlite_type, db_type='mysql'):
    """转换 SQLite 类型到目标数据库类型"""
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        for table_name in tables:
            ...
            for col in columns:
                ...
                mysql_type = convert_sqlite_type_to_target(col_type, 'mysql')
...
def main():
    ...
    if len(sys.argv) < 2:
        print("用法:")
        print("  python convert_sqlite_to_other.py export    # 导出 SQLite 为 SQL 文件")
        print("  python convert_sqlite_to_other.py import    # 导入 SQL 文件到目标数据库")
        print()
        print("示例:")
        print("  # 1. 导出")
        print("  python convert_sqlite_to_other.py export")
        print()
        print("  # 2. 导入（需要设置数据库密码）")
        print("  export DB_PASSWORD=your_password")
        print("  python convert_sqlite_to_other.py import")
        sys.exit(1)
