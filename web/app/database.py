...
def migrate_app_settings_default_converter():
    ...
    try:
        inspector = inspect(db.bind)
        cols = [c["name"] for c in inspector.get_columns("app_settings")]
        if "default_converter_id" not in cols:
            db.execute(
                text("ALTER TABLE app_settings ADD COLUMN default_converter_id VARCHAR(64) DEFAULT 'logics-parsing-v2'")
            )
            print("✅ 已添加 app_settings.default_converter_id 字段")

        # 兜底初始化空值，避免历史数据为 NULL
        db.execute(
            text(
                "UPDATE app_settings SET default_converter_id = 'logics-parsing-v2' "
                "WHERE default_converter_id IS NULL OR TRIM(COALESCE(default_converter_id, '')) = ''"
            )
        )
        db.commit()
    except Exception as e:
        db.rollback()
        print(f"⚠️ 迁移 default_converter_id 失败: {e}")
    finally:
        db.close()
...
def migrate_app_settings_drop_legacy_converter_columns():
    ...
    try:
        ...
        legacy_cols = [
            ...
            "paddle_visualize",
            "paddle_extra_options_json",
        ]
        dropped = []
        for col in legacy_cols:
            if col in cols:
                db.execute(text(f"ALTER TABLE app_settings DROP COLUMN {col}"))
                dropped.append(col)
