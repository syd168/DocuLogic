...
# 某些 Linux 发行版默认缺少 .mjs 映射，导致模块脚本被返回为 octet-stream。
...
from converts.middleware.contracts import ConversionJobInput
from converts.middleware.host import run_conversion_job
...
app.add_middleware(
    ...
    allow_methods=["GET", "POST", "PUT", "PATCH", "DELETE", "OPTIONS"],  # 含 PATCH（管理端用户更新等）
...
_model_lock = threading.Lock()  # 用于线程安全的模型加载 / 卸载
_last_used_time: Optional[float] = None  # 最后使用时间戳（加载、推理开始/结束）
_model_unload_timeout: int = 300  # 模型空闲超时时间（秒），默认5分钟
# 并发文档解析任务数；仅在为 0 且超时时才允许自动卸载，避免多任务下误卸模型
_model_inference_lock = threading.Lock()
_model_inference_active: int = 0 # 并发文档解析任务数；仅在为 0 且超时时才允许自动卸载，避免多任务下误卸模型


def begin_model_inference() -> None:
    """在即将占用模型进行推理前调用（与 end_model_inference 成对）。"""
    global _model_inference_active, _last_used_time
    with _model_inference_lock:
        _model_inference_active += 1
        _last_used_time = time.time()


def end_model_inference() -> None:
    """文档解析任务结束（含异常）时调用，与 begin_model_inference 成对。"""
    global _model_inference_active, _last_used_time
    with _model_inference_lock:
        if _model_inference_active > 0:
            _model_inference_active -= 1
        _last_used_time = time.time()


def _dispose_inference_model(m: LogicsParsingModel) -> None:
    """拆掉 LogicsParsingModel 内的 HF 权重与处理器引用，便于 GC 与 CUDA 释放显存。

    仅 ``del`` 外层包装时，内部 ``nn.Module`` 可能仍被引用环挂住；显式 ``to('cpu')`` + ``del``
    更接近 HuggingFace / PyTorch 常见卸载方式。``to('cpu')`` 在极低内存下可能失败，此时仍尝试 ``del``。
    """
    log = logging.getLogger("app")
    lm = getattr(m, "model", None)
    try:
        m.model = None
    except Exception:
        pass
    if lm is not None:
        try:
            ...
            log.warning("unload: model.to(cpu) skipped (%s)", e)
        try:
            del lm
        except Exception:
            pass
...
def _release_model_state_after_del() -> None:
    """在已拆掉模型对象后执行 GC / CUDA 缓存清理（不做加锁）。"""
    import gc
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            print(f"✅ 模型已卸载，当前 PyTorch 已分配显存约: {allocated:.2f}GB（进程仍保留缓存池属正常）")
    except Exception:
        pass
    print("✅ 模型已成功卸载，显存已释放")
...
def get_inference_model() -> Optional[LogicsParsingModel]:
    ...
    自动卸载仅在「无进行中的解析任务」且空闲超时时触发；加锁顺序与
    begin/end_model_inference 一致，避免与推理并发卸模型。
    """
    global model, _last_used_time

    with _model_inference_lock:
        if (
            model is not None
            and _last_used_time is not None
            and _model_inference_active == 0
            and (time.time() - _last_used_time) > _model_unload_timeout
        ):
            print(
                f"⏰ 模型已空闲 {time.time() - _last_used_time:.0f} 秒，正在卸载以释放显存..."
            )
            with _model_lock:
                if model is not None:
                    try:
                        ...
                        _dispose_inference_model(to_free)
                        del to_free
                        _release_model_state_after_del()
                    except Exception as e:
                        print(f"⚠️  卸载模型时出错: {e}")
                        model = None
                        _last_used_time = None
...
def ensure_model_loaded() -> LogicsParsingModel:
    """确保模型已加载，如果未加载则进行懒加载"""
    global model, _last_used_time
            ...
            try:
                ...
                try:
                    ...
                    if torch.cuda.is_available():
                        ...
                        print(f"✅ 模型懒加载成功! 显存使用: {allocated_after:.2f}GB (新增 {used:.2f}GB)")
...
def unload_model(
    *,
    wait_for_inference: bool = False,
    wait_timeout: float = 300.0,
) -> bool:
    """卸载模型以释放显存。

    若有进行中的解析任务，默认不卸载并返回 False；管理员重载可设
    wait_for_inference=True 在超时内等待任务结束后再卸载。

    返回 True 表示当前已无加载的模型（含本函数完成卸载或本就未加载）；
    返回 False 表示因仍有推理任务且未等待或等待超时而未卸载。
    """
    global model, _last_used_time
...
def reload_model_from_settings() -> None:
    """重新加载模型（用于管理接口调用）"""
    if not unload_model(wait_for_inference=True, wait_timeout=300.0):
        raise RuntimeError(
            "仍有解析任务在使用模型，在限期内未能完全空闲，无法安全卸载并重载。"
            "请等待任务完成或停止任务后再试。"
        )
    ensure_model_loaded()
...
async def startup_event():
    """应用启动时不加载模型，改为懒加载模式（首次使用时才加载）
    这样可以避免不必要的显存占用，只有在真正需要解析文档时才加载模型。"""
    global model
    ...
    print("💡 如需立即加载模型，可访问「系统设置」→「重新加载模型」\n")
...
def _pick_existing_job_file(job_dir: Path, *candidates: str) -> Optional[Path]:
    """按顺序返回第一个存在的任务文件，用于 .md/.mmd 兼容读取。"""
...
def _debug_mode_enabled() -> bool:
    return os.environ.get("DEBUG_MODE", "").lower() in ("1", "true", "yes")


def _is_user_stop_process_exception(exc: BaseException) -> bool:
    """与 model_inference 在用户停止时抛出的 ValueError 前缀一致，避免子串误判。"""
    return isinstance(exc, ValueError) and str(exc).startswith("已停止：")
...
async def process_document_task(
    ...
    try:
        ...
        default_engine_id = (
            getattr(settings_row, "default_converter_id", "logics-parsing-v2") if settings_row else "logics-parsing-v2"
        )
        default_engine_id = (str(default_engine_id or "").strip() or "logics-parsing-v2")
        ...
        try:
            if default_engine_id == "logics-parsing-v2":
                ...
                job_input = ConversionJobInput(
                    ...
                    payload={
                        ...
                        "converter_runtime_config": converter_runtime_config,
                    },
                    progress_callback=progress_callback,
                    engine_context={},
                )
                result = await asyncio.to_thread(run_conversion_job, job_input)
                result = result.data
            ...
            # 检查是否是用户主动停止
            is_user_stopped = _is_user_stop_process_exception(e)
            
            if is_user_stopped:
                import logging
                logging.getLogger("app").info("任务 %s 用户主动停止: %s", job_id, e)
                ...
                if job_dir.exists():
                    ...
                    raw_md = _pick_existing_job_file(job_dir, f"{job_id}_raw.md", f"{job_id}_raw.mmd")
                    markdown_md = _pick_existing_job_file(job_dir, f"{job_id}.md", f"{job_id}.mmd")
                    result_zip = job_dir / f"{job_id}_result.zip"  # 新增：检查完整结果 ZIP
                    ...
                    if raw_md:
                        output_files["raw"] = str(raw_md)
                    if markdown_md:
                        output_files["markdown"] = str(markdown_md)
                    
                    # 如果有完整结果 ZIP，添加到 output_files
                    if result_zip.exists():
                        output_files["download_zip"] = str(result_zip)
                ...
                if _debug_mode_enabled():
                    print(f"[调试] 任务 {job_id} 处理异常: {type(e).__name__}: {e}")
                    traceback.print_exc()
...
async def api_settings(
    ...
    out = {
        ...
        "model_loaded": get_inference_model() is not None,  # 使用懒加载函数
...
async def list_jobs(
    ...
    for r in rows:
        ...
        item = {
            ...
            and has_files,
            "has_result_zip": has_result_zip,  # 新增：标识是否有完整结果 ZIP
        }
        if getattr(current_user, "is_admin", False):
...
async def download_result(
    ...
    if file_type not in {"raw", "markdown", "result"}:
    ...
    if file_type == "result":
        ...
        file_path = _pick_existing_job_file(job_dir, f"{job_id}_raw.md", f"{job_id}_raw.mmd")
        media = "text/plain; charset=utf-8"
        if not file_path:
            raise HTTPException(status_code=404, detail="File not found")
    else:
        file_path = _pick_existing_job_file(job_dir, f"{job_id}.md", f"{job_id}.mmd")
        media = "text/plain; charset=utf-8"
        if not file_path:
            raise HTTPException(status_code=404, detail="File not found")
...
async def health_check(db: Session = Depends(get_db)):
    ...
    # 检查模型状态（不影响整体健康状态）
    checks["model_loaded"] = get_inference_model() is not None  # 使用懒加载函数
...
async def batch_download_results(
    ...
    参数：
        job_ids: 任务 ID 列表（JSON 对象字段，与前端一致）
    ...
    try:
        with zipfile.ZipFile(temp_zip.name, 'w', zipfile.ZIP_DEFLATED) as zipf:
            for job_id, job_row, job_dir in valid_jobs:
                ...
                # 添加 Markdown 文件
                md_file = _pick_existing_job_file(job_dir, f"{job_id}.md", f"{job_id}.mmd")
                if md_file:
                    arc_name = f"{task_prefix}/{md_file.name}"
                    zipf.write(md_file, arc_name)
                
                # 添加原始输出文件
                raw_file = _pick_existing_job_file(job_dir, f"{job_id}_raw.md", f"{job_id}_raw.mmd")
                if raw_file:
                    arc_name = f"{task_prefix}/{raw_file.name}"
