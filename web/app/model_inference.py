...
def _is_user_stop_exception(exc: BaseException) -> bool:
...
class LogicsParsingModel:
    DEFAULT_ENGINE_ID = "logics-parsing-v2"
    ...
    def _load_model(self):
        ...
        # 检查环境变量配置
        use_8bit = os.environ.get("MODEL_USE_8BIT", "0").lower() in ("1", "true", "yes")
        force_cpu = os.environ.get("MODEL_FORCE_CPU", "0").lower() in ("1", "true", "yes")
        ...
        if torch.cuda.is_available() and not force_cpu:
            ...
            # 根据显存大小自动选择策略
            if gpu_memory < 12:
                # 显存小于 12GB，强制使用 8-bit 量化
                if not use_8bit:
                    print(f"⚠️  检测到显存较小 ({gpu_memory:.1f} GB)，自动启用 8-bit 量化以节省显存")
                    use_8bit = True
            
            if use_8bit:
                device_map = "auto"  # 使用 auto 让 Accelerate 智能分配
                dtype = torch.float16
                attn_impl = "sdpa"
                load_in_8bit = True
                
                print(f"✅ 检测到 GPU (8-bit 量化模式):")
                print(f"   - 型号: {gpu_name}")
                print(f"   - 显存: {gpu_memory:.1f} GB")
                print(f"   - CUDA 版本: {torch.version.cuda}")
                print(f"   - 计算精度: float16 + 8-bit 量化")
                print(f"   - 注意力机制: sdpa (加速)")
                print(f"   - 设备分配: auto (智能分配)")
            else:
                device_map = "cuda:0"
        ...
        # 如果启用 8-bit 量化，添加相关参数
        if use_8bit:
            try:
                import bitsandbytes
                model_kwargs["load_in_8bit"] = True
                model_kwargs["llm_int8_enable_fp32_cpu_offload"] = True  # 允许部分层在 CPU
                print(f"✅ 已启用 8-bit 量化（需要 bitsandbytes 库）")
            except ImportError:
                print(f"⚠️  bitsandbytes 库未安装，回退到标准模式")
    ...
    def plot_bbox(self, img_path: str, pred: str, output_path: str):
        ...
        if img is None:
            logging.getLogger("app").warning(
                "plot_bbox: cv2.imread 失败，无法绘制框: %s", img_path
            )
    ...
    def process_document(
        ...
        """通过中间件调度处理文档，保持原有入参与返回结构不变。"""
        selected_engine_id = (engine_id or self.DEFAULT_ENGINE_ID).strip()
        job_input = ConversionJobInput(
            ...
            engine_context={
                # 第一阶段先复用现有逻辑，确保接口/行为不变；
                # 后续插件可逐步替换为独立实现。
                "legacy_process_fn": self._process_document_legacy,
            },
        )
        return run_conversion_job(job_input).data

    def _process_document_legacy(
        self,
        input_path: str,
        output_dir: str,
        job_id: str,
        ...
        try:
            ...
            # 用户主动停止不记错误日志，由上层统一按 stopped 处理
            if _is_user_stop_exception(e):
                raise

            # 其他异常：错误信息脱敏，记录详细日志，返回友好提示
            import logging
    ...
    def _process_single_image(
        ...
        output_mmd_path = os.path.join(output_dir, f"{job_id}.md")
        ...
        if image_output_mode == "separate":
            ...
            with zipfile.ZipFile(download_zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
                # 添加 markdown 文件
                zf.write(output_mmd_path, arcname=f"{job_id}.md")
                
                # 添加 raw 文件
                zf.write(output_raw_path, arcname=f"{job_id}_raw.md")
