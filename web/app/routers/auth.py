...
async def refresh_token(
    ...
    # 同步刷新会话里的 token，避免后续被单点登录校验误判为“其他设备登录”。
    refresh_session_token(uid, new_token, timeout_minutes * 60)
...
@router.get("/session")
async def session_status(user: User | None = Depends(get_current_user_optional)):
    """
    静默会话探测接口：
    - 未登录时返回 200 + authenticated=false（避免首页出现 401 噪声）
    - 已登录时返回最小用户信息
    """
    if user is None:
